...
typedef struct _options {
  ...
  uint8_t in_fmt,
      out_fmt; // fmts 0 = undef, 1 = bin, 2 = xml, 3 = json, 4 = openstep
  uint8_t flags;
} options_t;
...
static void print_usage(int argc, char *argv[]) {
  ...
  printf("\n");
  printf(
      "Convert a plist FILE between binary, XML, JSON, and OpenStep format.\n");
  printf("If -f is omitted, XML plist data will be converted to binary and "
         "vice-versa.\n");
  printf("To convert to/from JSON or OpenStep the output format needs to be "
         "specified.\n");
  printf("\n");
  printf("OPTIONS:\n");
  printf("  -i, --infile FILE    Optional FILE to convert from or stdin if - "
         "or not used\n");
  printf("  -o, --outfile FILE   Optional FILE to convert to or stdout if - or "
         "not used\n");
  printf(
      "  -f, --format FORMAT  Force output format, regardless of input type\n");
  printf(
      "                       FORMAT is one of xml, bin, json, or openstep\n");
  printf(
      "                       If omitted, XML will be converted to binary,\n");
  printf("                       and binary to XML.\n");
  printf("  -p, --print FILE     Print the PList in human-readable format.\n");
  printf("  -c, --compact        JSON and OpenStep only: Print output in "
         "compact form.\n");
  printf("                       By default, the output will be "
         "pretty-printed.\n");
  printf("  -s, --sort           Sort all dictionary nodes lexicographically "
         "by key\n");
...
static options_t *parse_arguments(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "--infile") || !strcmp(argv[i], "-i")) {
      ...
      } else if (!strncmp(argv[i + 1], "openstep", 8) ||
                 !strncmp(argv[i + 1], "ostep", 5)) {
...
int main(int argc, char *argv[]) {
  ...
  if (options->out_fmt == 0) {
    ...
    if (input_res == PLIST_ERR_SUCCESS) {
      ...
      if (options->out_fmt == PLIST_FORMAT_BINARY) {
        ...
        output_res = plist_to_json(root_node, &plist_out, &size,
                                   !(options->flags & OPT_COMPACT));
      } else if (options->out_fmt == PLIST_FORMAT_OSTEP) {
        output_res = plist_to_openstep(root_node, &plist_out, &size,
                                       !(options->flags & OPT_COMPACT));
      } else {
        plist_write_to_stream(root_node, stdout, options->out_fmt,
  ...
  if (input_res == PLIST_ERR_SUCCESS) {
    ...
    case PLIST_ERR_PARSE:
      if (options->out_fmt == 0) {
        fprintf(stderr, "ERROR: Could not parse plist data, expected XML or "
                        "binary plist\n");
      } else {
